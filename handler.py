...
def get_edit_params(request):
    ...
    stop_date = request['stop_date']
    start_date = request['start_date']
    update_db(proxy_port_in, fp_name, author, stop_date, start_date, proxy_port_out, project, proxy_name)


def update_db(proxy_port_in, fp_name, author, stop_date, start_date, \
              proxy_port_out, project, proxy_name):
    session.query(ProxyList).\
